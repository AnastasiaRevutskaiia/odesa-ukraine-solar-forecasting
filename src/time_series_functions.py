...
def load_data_solar_hours(path, min_max, use_log, save_cv):
    ...
    df_total['Ghi'] = df_total['Ghi']
    df_total['PeriodEnd'] = pd.to_datetime(df_total['PeriodEnd'], format='%Y-%m-%dT%H:%M:%S+00:00')

    df_total = df_total[['PeriodEnd', 'Ghi']]

    df_total.rename(columns={'Ghi': 'actual'}, inplace=True)
    df_total.rename(columns={'PeriodEnd': 'Data'}, inplace=True)
